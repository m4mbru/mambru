...
fn main() {
    ...
    // `/utf-8` is an MSVC-specific flag.  When targeting `*-pc-windows-gnu`
    // (MinGW/GCC) we MUST NOT pass it.  We use the runtime TARGET env var
    // because cfg!() in build.rs reflects the HOST, not the target.
    ...
    // ── PATCH for MinGW compatibility ──
    // CMake/MinGW builds ggml static libraries as `ggml.a` (no `lib` prefix),
    // but rustc's `-l static=ggml` searches for `libggml.a`.
    // Symlink/copy ggml.a → libggml.a so the linker can find them.
    if target.contains("windows") && target.contains("gnu") {
        let ggml_src = out.join("build").join("ggml").join("src");
        for name in &["ggml.a", "ggml-base.a", "ggml-cpu.a"] {
            let src = ggml_src.join(name);
            let dst = ggml_src.join(format!("lib{}", name));
